...
class Furniture(object):
    ...
    def load(self,_filename,name):
        ...
        self.angle     = eval(config.get(name,"angle"))
        self.height    = eval(config.get(name,"height"))
        self.width     = eval(config.get(name,"width"))
        self.length    = eval(config.get(name,"length"))
        self.thickness = eval(config.get(name,"thickness"))
    ...
    def save(self,_filename):
        ...
        sorigin = '[ '+str(self.origin[0])+' , '+str(self.origin[1])+ ' ]'
    ...
    def position(self):
        ...
        position = []
        p0  = self.origin
        u   = np.array([np.cos(self.angle*np.pi/180),np.sin(self.angle*np.pi/180)])
        v   = np.array([-np.sin(self.angle*np.pi/180),np.cos(self.angle*np.pi/180)])
        p1  = p0 + u * self.length
        p2  = p1 + v * self.width
        p3  = p2 - u * self.length
    ...
    def show(self,fig=[],ax=[],offx=0,offy=10):
        ...
        if self.Matname=='METAL':
            pathpatch = PathPatch(path, facecolor='grey', edgecolor='blue',alpha=0.5)
        else:
            pathpatch = PathPatch(path, facecolor='green', edgecolor='black',alpha=0.2)
