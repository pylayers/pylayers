    ...
    if s1.intersects(s2):
        ...
        if s1.touches(s2): # touching segments
            if not ( pi.equals_exact(p1t,tol) or pi.equals_exact(p1h,tol) ):
                s11 = shg.LineString(((p1t.xy[0][0],p1t.xy[1][0]),(pi.xy[0][0],pi.xy[1][0])))
                s12 = shg.LineString(((pi.xy[0][0],pi.xy[1][0]),(p1h.xy[0][0],p1h.xy[1][0])))
            ...
            if not ( pi.equals_exact(p2t,tol) or pi.equals_exact(p2t,tol) ): 
                s21 = shg.LineString(((p2t.xy[0][0],p2t.xy[1][0]),(pi.xy[0][0],pi.xy[1][0])))
                s22 = shg.LineString(((pi.xy[0][0],pi.xy[1][0]),(p2h.xy[0][0],p2h.xy[1][0])))
...
def parsevrml(filename): 
    ...
    for li in lignes:
        ...
        if li.find('DEF')!=-1:      # if line contains DEF keyword new dictionnary entry 
            k    = k+1
            tmp  = li.split(' ')    # split with space the index 1 is the dictionnary entry name 
            obid = tmp[1]
            tobj[k]=li
            tnum[k]=obid            # name of the entry 
        else:
            try:
                tobj[k]=tobj[k]+li  # concatenation of the line in the current dictionnary entry k 
            except:
                pass
    td = {}        
    dg = {}                         # list of groups
        ...
        for g in group:
            for ID in self.entity[g]:
                for p in self.entity[g][ID]['poly']:
                    ...
                    self.entity[g][ID]['poly'][p].plot(color=colrand,alpha=0.3)
        plt.axis('scaled')
