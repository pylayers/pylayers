...
class Agent(object):
    ...
    def __init__(self,**args):
        defaults = {'ID': 0,'type':'ag','pos':np.array([]),'roomId':0, 'meca_updt':0.1,'loc':False,'loc_updt':0.5,'Layout':Layout(),'net':Network(),'RAT':['wifi'],'world':world(),'sim':Simulation}
        ...
        # mecanique
        if self.type == 'ag':
            self.meca=Person3(ID=self.ID,roomId=args['roomId'],L=args['Layout'],net=self.net,interval=args['meca_updt'],wld=args['world'],sim=args['sim'],moving=True)
