...
def ptw1():
    ...
    Rx[1,:] = np.array([298622.3689,2354082.0733])
    Rx[2,:] = np.array([298617.4193,2354088.4029])
    Rx[3,:] = np.array([298614.2383,2354080.9762])
    Rx[4,:] = np.array([298607.736,2354088.391])
...
class Tdd(object):
    ...
    def PL(self,fmin,fmax,B):
        ...
        N    = len(freq)
        i_start = np.nonzero((S1.x>=fmin-df/2.)&(S1.x<fmin+df/2.))
        i_stop  = np.nonzero((S1.x>=fmax-df/2.)&(S1.x<fmax+df/2.))
        f = S1.x[i_start[0][0]:i_stop[0][0]]
        for k in range(N):
            u  = np.nonzero((f>=fmin+k*B )& (f<fmin+(k+1)*B))
            Et  = 10*np.log10(np.sum(Stx.y[u[0]]*np.conj(Stx.y[u[0]]))).astype('float')
            Er1 = 10*np.log10(np.sum(S1.y[u[0]]*np.conj(S1.y[u[0]]))).astype('float')
            Er2 = 10*np.log10(np.sum(S2.y[u[0]]*np.conj(S2.y[u[0]]))).astype('float')
            Er3 = 10*np.log10(np.sum(S3.y[u[0]]*np.conj(S3.y[u[0]]))).astype('float')
            Er4 = 10*np.log10(np.sum(S4.y[u[0]]*np.conj(S4.y[u[0]]))).astype('float')
            PL1 = Et - Er1
            PL2 = Et - Er2
            PL3 = Et - Er3
            PL4 = Et - Er4
    ...
    def show(self,\
        ...
        self.ch1.plot(col=col[0],vline=np.array([delay[0]]),showlabel=[False,True],unit1='V',unit2='mV',ax=sp1)
        ...
        if createfig:
            ...
            sp2=ax[2]
        self.ch2.plot(col=col[1],vline=np.array([delay[1]]),showlabel=[False,True],unit1='V',unit2='mV',ax=sp2)
        ...
        if createfig:
            ...
            sp3=ax[3]
        self.ch3.plot(col=col[2],vline=np.array([delay[2]]),showlabel=[False,True],unit1='V',unit2='mV',ax=sp3)
        ...
        if createfig:
            ...
            sp4=ax[4]
        self.ch4.plot(col=col[3],vline=np.array([delay[3]]),showlabel=[True,True],unit1='V',unit2='mV',ax=sp4)
    ...
    def show_span(self,delay=np.array([0,0,0,0]),wide=np.array([0,0,0,0])):
        ...
        plt.axvspan(delay[0]-wide[0]/2, delay[0]+wide[0]/2, facecolor='g', alpha=0.5)
        plt.xlabel('Time (ns)')
        plt.title('Rx1')
        sp2=fig.add_subplot(222)
        plt.plot(self.ch2.x,self.ch2.y)
        plt.axvspan(delay[1]-wide[1]/2, delay[1]+wide[1]/2, facecolor='g', alpha=0.5)
        plt.xlabel('Time (ns)')
        plt.title('Rx2')
        plt.sp3=fig.add_subplot(223)
        plt.plot(self.ch3.x,self.ch3.y)
        plt.axvspan(delay[2]-wide[2]/2, delay[2]+wide[2]/2, facecolor='g', alpha=0.5)
        plt.xlabel('Time (ns)')
        plt.title('Rx3')
        sp4=fig.add_subplot(224)
        plt.plot(self.ch4.x,self.ch4.y)
        plt.axvspan(delay[3]-wide[3]/2, delay[3]+wide[3]/2, facecolor='g', alpha=0.5)
        plt.xlabel('Time (ns)')
        plt.title('Rx4')
...
class TFP(object):
    ...
    def append(self,FP):
        ...
        self.metadata['distance']=hstack((self.metadata['distance'],FP.metadata['distance']))
        self.metadata['delay']=hstack((self.metadata['delay'],FP.metadata['delay']))
        self.metadata['los_cond']=hstack((self.metadata['los_cond'],FP.metadata['los_cond']))
        self.metadata['angular']=hstack((self.metadata['angular'],FP.metadata['angular']))


        self.chan_param['Etot']=hstack((self.chan_param['Etot'],FP.chan_param['Etot']))
        self.chan_param['Emax']=hstack((self.chan_param['Emax'],FP.chan_param['Emax']))
        self.chan_param['Etau0']=hstack((self.chan_param['Etau0'],FP.chan_param['Etau0']))
        self.chan_param['tau_moy']=hstack((self.chan_param['tau_moy'],FP.chan_param['tau_moy']))
        self.chan_param['tau_rms']=hstack((self.chan_param['tau_rms'],FP.chan_param['tau_rms']))
        self.chan_param['Epercent']=hstack((self.chan_param['Epercent'],FP.chan_param['Epercent']))
        self.chan_param['toa_max']=hstack((self.chan_param['toa_max'],FP.chan_param['toa_max']))
        self.chan_param['toa_th']=hstack((self.chan_param['toa_th'],FP.chan_param['toa_th']))
        self.chan_param['toa_cum']=hstack((self.chan_param['toa_cum'],FP.chan_param['toa_cum']))
        ...
        self.chan_param['toa_win']=hstack((self.chan_param['toa_win'],FP.chan_param['toa_win']))
        self.chan_param['toa_new']=hstack((self.chan_param['toa_new'],FP.chan_param['toa_new']))
...
class FP(object):
    ...
    F = FP(M,Rxid, alpha = 0.1,Tint=0.225,sym=0.25,nint=7,thlos=0.05,thnlos=0.65,thcum=0.15)
    Rxid : 1 | 2 | 3 | 4
...
class UWBMesure(object):
    ...
    Notes
    -----
    Extracted from Deliverable D4.1 WHERE1 Project

    When performing channel sounding, it is necessary to extract exclusively
    the channel behavior (with or without antennas) by using appropriate calibration
    procedures. We have decided to include antenna effects in the measurements.
    The calibration procedure is mainly based on deconvolution tools described hereafter.
    The transmitter link up to the Tx antenna is exactly the same for all channel measurements.
    However, as previously explained, during the measurement campaign,
    the four receiver branches are not identical.

    Consequently, the calibration differs from one branch to another.
    All the elements involved in the link are deconvoluted out of measurements (PA, cables, LNAs…).

    Deconvoluting the effect of the components is performed in the frequency domain
    thanks to a simple complex spectrum ratio.

    The frequency signal is then passed back to the time domain by a Passband Hermitian
    Reconstruction (PHR). A complete description of this inverse Fourier transform (IFT)
    method is available in [DK03].

    For all channel measurements, this pre-processing allows us to exclusively get
    the convolution of each of the four channel impulse responses with both antenna
    responses (called CIR). Moreover, the compound response in time domain, corresponding to
    the convolution between the CIR and the transmitted pulse shape feeding TX antenna
    is easily available.

    Pre-processing steps for impulse feeding Tx antenna:

        + Time domain calibration is done (RAW_DATA.TX) for getting impulse feeding
        Tx antenna, but with an attenuator called Attenuator1.

        The corresponding time scale is RAW_DATA.TimeTX.
        + Time domain data (RAW_DATA.TX) is transformed to frequency domain thanks
        to Fast Fourier Transform.

        + Deconvolution of attenuator is done in frequency domain, in order to get
        exact impulse spectrum feeding Tx antenna (DECONV_DATA.FREQ_DOMAIN.TX).
        The corresponding frequency scale is DECONV_DATA.FREQ_DOMAIN.Freq.

        + We transform back data into time domain through Inverse Fast Fourier transform
        (IFFT) thanks to PHR, and finally we get DECONV_DATA.TIME_DOMAIN.TX.
        The corresponding time scale is DECONV_DATA.TIME_DOMAIN.Time.

        + Finally, this signal corresponds to impulse feeding Tx antenna
        (see Figure 3.15) in time    domain.

       Pre-processing steps for calibration signals:
        + Time domain calibrations are done (CAL_DATA.CHi  with i={1,2,3,4}) for the
        four channels links without antennas by simply including attenuators,
         in place of antennas and channel, called Attenuator1 and Attenuator2.
        The corresponding time scale is RAW_DATA.Time.

        + Attenuators (CAL_DATA.VNA_Attenuator1 and CAL_DATA.VNA_Attenuator2)
        are measured thanks to a VNA in frequency domain from 1GHz up to 9GHz
        with a 5MHz step. The corresponding frequency scale is CAL_DATA.VNA_Freq.

        + Time domain data (CAL_DATA.Chi) are transformed to frequency domain thanks
         to Fast Fourier Transform.
        + Deconvolution of attenuators is done in frequency domain, in order
        to get exact transfer function of all Tx and Rx components for the four channels.


        Pre-processing step for raw signals:
        + For each Tx position, we get four raw impulse received from oscilloscope
        (RAW_DATA.CHi with i={1,2,3,4}) with antennas and channel behaviour.
        The corresponding time scale is RAW_DATA.Time.
        + Time domain data (RAW_DATA.CHi) are transformed to frequency domain thanks to
        Fast Fourier Transform.
        + Deconvolution of all Tx and Rx components (from last step of calibration signal
        s pre-processing) is done in frequency domain, in order to get the transfer function
         in frequency domain from Rxi (DECONV_DATA.FREQ_DOMAIN.CHi).
         This complex transfer function is exclusively channel and antennas behaviour.
         The corresponding frequency scale is DECONV_DATA.FREQ_DOMAIN.Freq.
        + Finally, these signals correspond to CIR (Channel Impulse Response including antennas)
         in frequency domain.

        Warnings
        --------
        Caution: Results outside the 3-7GHz bandwidth is unusable without any windowing
        or convolution with a pulse shape in the bandwidth!

        Notes
        -----
        + Convolution between the CIR (DECONV_DATA.FREQ_DOMAIN.CHi) and the transmitted
        pulse shape feeding TX antenna (DECONV_DATA.FREQ_DOMAIN.TX) is done in order
         to get the compound responses in frequency domain.
        We transform back data into time domain through Inverse Fast Fourier transform
        (IFFT) thanks to PHR, and finally we get the compound response from Rxi
        (DECONV_DATA.TIME_DOMAIN.CHi).

        The corresponding time scale is DECONV_DATA.TIME_DOMAIN.Time.
        + Finally, these signals correspond to compound responses in time domain.


        LQI Method 1: for each compound response, the ratio between the maximum
        signal amplitude and the maximum noise amplitude (estimated during the first
        five nanoseconds corresponding to a common time slot without any signal)
        in dB scale is performed.

        LQI Method 2: for each compound response, the following calculation is done.
        Firstly, the signal and noise energy is calculated over the whole time window
        (200ns). Second, noise variance is estimated during the first five nanoseconds
        corresponding to a common time slot without any signal, and multiplied by the
        total observation time (200ns). Thus, we get the total noise energy, which can be
        subtracted from signal and noise energy, in order to get only signal energy.
        And finally, the ratio between the signal energy and the noise energy in dB
        scale is performed.
...
#            print "Detruit: ",self.Tx_position
    ...
    def show(self,
        ...
        self.tdd.show(delay,\
                      display=display,\
                      titre=['Rx1 '+'Tx'+str(self.ntx),'Rx2','Rx3','Rx4'],\
                      col=col,\
                      xmin=xmin,\
                      xmax=xmax,\
                      C=C,\
    ...
    def tau_Emax(self):
                ...
                tau_Emax = np.array([tau_Emax1,tau_Emax2,tau_Emax3,tau_Emax4])
    ...
    def Etot(self):
        ...
        etot = np.array([10*log10(Etot1),10*log10(Etot2),10*log10(Etot3),10*log10(Etot4)])
    ...
    def tdelay(self):
        ...
        t1=vstack((tau0,toa_th,toa_cum,toa_max,tau_moy-tau_rms,tau_moy+tau_rms))
        t2=t1.T
        return(t2)
