...
M = MIMO('S00_11.csv','12-9-2013/S0/',calibration=True);
D = M-V
##M.h.plot(types='m',dist=True)
#
...
scat = ax1.scatter(D.grid[...,0],D.grid[...,1],c=abs(D.gloc[:,769]),s=100,vmin=0,vmax=0.001,linewidth=0)
delay_template = '%d : tau = %5.2f (ns) d= %5.2f (m)'
delay_text  = ax1.text(0.25,0.9,'',transform=ax1.transAxes,fontsize=18)
# initialization function: plot the background of each frame
def init():
    delay_text.set_text('')
    scat.set_array(abs(D.gloc[:,769]))
    return scat,delay_text

# animation function.  This is called sequentially
def animate(i):
    delay_text.set_text(delay_template%(780+i,t[780+i],t[780+i]*0.3))
    scat.set_array(abs(D.gloc[:,769+i]))
    return scat,delay_text
    #return delay_text,
