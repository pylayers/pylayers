...
#from rpy import r
#
#  ToDo sauvegarde matlab
#
class Bsignal(object):
    """ BSignal : (Based Signal) Signal with an embedded time base

    This class gathers a 1D signal and its axis indexation. 
    
    This class offers a transparent back and forth between time and frequency domain.
    
    Bsignal stands for Based signal = a signal with a base
    ...
    def __init__(self,x=np.array([]),y=np.array([])):
        ...
        Warnings
        --------
            By construction len(y)=len(x) len(x) has priority
        """
        self.x = x
        self.y = y
        ndim = self.y.ndim
    ...
    def plot(self,ix=0,col='black',vline=np.array([]),hline=np.array([]),unit1='V',unit2='V',xmin=-1e5,xmax=1e5,ax=[],dB=False,dist=False,display=True,logx=False,logy=False):
        ...
        Parameters 
        ----------
        ix    : index of the waveform to plot (-1 = all)
        col   : string 
            default  'black'
        vline : ndarray
        hline : ndarray
...
class Usignal(Bsignal):
    ...
    def width(self):
        ...
        By convention equal to the difference between extremities + dx 
        usage : width()
        """
...
class TBsignal(Bsignal):
    ...
    def plot(self,ix=0,col='black',vline=np.array([]),hline=np.array([]),showlabel=[True,True],unit1='V',unit2='V',ax=[],tmin=-1e5,tmax=+1e5,dB=False,dist=False,logx=False,logy=False):
        ...
        if tmax!= 1e5:
            ...
            xmax = tmax
        Bsignal.plot(self,ix=ix,col=col,vline=vline,hline=hline,unit1=unit1,unit2=unit2,xmin=xmin,xmax=xmax,ax=ax,dB=dB,dist=dist,logx=logx,logy=logy)
        if showlabel[0]==True:
            if dist:
    ...
    def translate(self,tau):
        """ 
        translate : translate the signal by tau 
        ...
        Once translate both Usignal.x might be not on the same grid
        Need to align both 
        """
        self.x = self.x + tau
...
class TUsignal(TBsignal,Usignal):
    """
        Uniform signal in Time domain hh
    ...
    def __add__(self,u):
        ...
        t=type(u).__name__
        if ((t=='int') | (t=='float')):
            U =TUsignal(self.x,self.y+u)
        else:
                L =self.align(u)
                u1=L[0]
                u2=L[1]
                U =TUsignal(u1.x,u1.y+u2.y)
        return(U)
...
#          #show()
    ...
    def shift(self,tau):
        ...
        Summary
        --------
        This method use fft and ifft to produce a shift which is not necessarily a
    ...
    def correlate(self,s,normalized=True):
        ...
        -------
                ...
                in the self signal by correlating with signal s, maximum of
...
#print shape(t_left),shape(x)
    ...
    def corrgauss(self,sigma):
        ...
        gn = TUsignal(t,1./(np.sqrt(2*np.pi)*sigma)*np.exp(-(t**2)/(2*sigma**2)))
    ...
    def Yadd_zeros2l(self,N):
        """ time domain extension on left with N zeros
        """
        te=self.dx()
...
#       u  = nonzero((tau0 + Tint*(1-sym) > self.x) & (self.x > tau0 - Tint*sym))
    ...
    def Efirst(self,toa,Tint=1,sym=0.25,dB=True):
        ...
        Efirst : Energy amount in the window (in dB if dB==True)
    ...
    def Efirst_toath(self,tau0,Tint = 1, sym = 0.25,dB = True):
        ...
        u = np.nonzero((toa + Tint*(1-sym) > self.x) & (self.x > toa - Tint*sym))
    ...
    def Emax(self,Tint=1,sym=0.5,dB=False): 
        ...
        and a symetry of sym around the max value of the squared signal 

        Emax(self,Tint=0.225,sym=0.25): 
        
        #  In the CEA-SIRADEL measurement 
        #  te     = 0.005 ns
        #  Nleft  = 12
        #  Nright = 33
        #  Tint   = 45*te = 0.225 ns 
    ...
    def tau_Emax(self):
        """
            Calculate the delay of Energy peak $\tau_s$
        """
        y2 = (self.y)**2
        t = self.x
        maxy2 = max(y2)
    ...
    #       while v < 50:
        ...
        plt.plot(1-THRE/M,V,'b',drawstyle='steps',label='interval number')
        plt.plot(1-THRE/M,VL,'-r',drawstyle='steps',label='interval(Left) number')
    ...
    def toa_cum(self,th):
        ...
        cdf,vary = self.ecdf()
        #
        #In the CEA-SIRADEL measurement th=0.15 
        #
        v   = plt.find(cdf.y>=th)
        toa = t[v[0]]
        return toa
    ...
    def toa_th_tmtm(self):
        ...
        alpha = (np.sqrt(self.Etot())-np.sqrt(self.Emax()))/(np.sqrt(self.Etot())+np.sqrt(self.Emax()))
        th  = alpha*maxy2
    ...
    def toa_th_tmt(self):
        ...
        alpha = (np.sqrt(self.Etot())-np.sqrt(self.Emax()))/np.sqrt(self.Etot())
        print alpha
        th  = alpha*maxy2
    ...
    def toa_cum_tmtm(self):
        ...
        alpha = (np.sqrt(cdf.y[-1])-np.sqrt(cdf.y[u]))/(np.sqrt(cdf.y[-1])+np.sqrt(cdf.y[u]))
        v   = plt.find(cdf.y>=alpha*cdf.y[u])
        toa = t[v[0]]
        return toa
    ...
    def readcea(self,_filename): 
        """
            eead  Waveform from CEA-LETI Matlab file
        """
    ...
    def ecdf(self,Tnoise=10,rem_noise=True,in_positivity=True,display=False,normalize=True,delay=0):
        """
        ecdf      : calculate energy cumulative density function 
        ...
        rem_noise  : remove noise if True
        in_positivity : inforce positivity if True
        normalize  : normalize if True (Not implemented)
        display    : display ecdf if True
        delay      : give a delay for vizualization 
...
class TUDsignal(TUsignal):
    ...
    def fig(self,N):
        """ plot a figure of th N first signals
    ...
    def plotdB(self,mask=False,n=2,phase=True):
        ...
        if ndim > 1 : 
            ...
            for k in range(nl):
                if phase:
                    plt.subplot(211)
                    plt.plot(self.x,10*n*np.log10(abs(self.y[k])+1.0e-15))
                    plt.xlabel('Frequency (GHz)')
            ...
            if mask==True:
                n = 1
                xfcc  = np.array([0.,0.96,0.96,0.96,0.96,1.61,1.61,1.61,1.61,1.99,1.99,1.99,1.99,3.1,3.1,3.1, 3.1,10.6,10.6,10.6,10.6,15])
                xcept = np.array([0.,1.61,1.61,1.61,1.61,3.8,3.8,3.8,3.8,6. ,6  , 6   ,6    ,8.5  ,8.5  ,8.5,8.5,10.6,10.6,10.6,10.6,15])
                ycept =np.array([-90,-90 ,-90 ,-85 ,-85 ,-85,-85,-70,-70,-70,-70,-41.3,-41.3,-41.3,-41.3,-65,-65,-65 ,-65 ,-85 ,-85,-85])
                yfcc=np.array([-41.3,-41.3,-41.3,-75.3,-75.3,-75.3,-75.3,-53.3,-53.3,-53.3,-53.3,-51.3,-51.3,-51.3,-51.3,-41.3,-41.3,-41.3,-41.3,-51.3,-51.3,-51.3])
...
class FUsignal(FBsignal,Usignal):
    ...
    *
    ...
    def newdf(self,df):
        """
            Resample the FUsignal through phase and module interpolation 
        ...
        mod_new = fm(f_new)
        arg_new = fa(f_new) 
        y_new   = mod_new*(np.cos(arg_new)+1j*np.sin(arg_new))*np.exp(-1j*f_new*2*np.pi*100)
    ...
    def symH(self,parity):
        ...
        if parity==0:
            if ndim > 1 :
                ...
                Up=np.concatenate((ze_y,U,np.flipud(np.conjugate(U[0:-1]))),0)
    ...
    def show(self):
        """
        show : pcolor visualization of Modulus and Phase
        """
        N  = np.shape(self.y)[0]
        tt = np.arange(N)
        plt.subplot(121)
...
#              r.lines(x,yi,col='black')
    ...
    def decimate(self,N):
        """
        decimate(N) : decimate FUsignal by N 
        """
        x = self.x
        y = self.y
...
class FUDsignal(FUsignal):
    ...
    iftd(Nz=1,tstart=-10,tstop=100,ffts=0)
    ...
    def ft2(self,df=0.01):
        """
        CIR : methode frequentielle. La fonction retourne la fonction de transfert 
              du canal en regime harmonique. 
        
        ft2(df=0.01)   : df pas frequentiel 0.01 GHz =  10 MHz 
...
class FHsignal(FUsignal):       
    ...
    def ifft(self,ffts=0,tt='centered'):
        ...
        Tww   = 1.0/df
        if (tt=='centered'):
            t     = np.linspace(-0.5*Tww+te/2,0.5*Tww+te/2,Np,endpoint=False)
        else:
