...
#	List of elementary Constraints 
	...
	def merge2(self,vcw_init=1.0):
		...
		while (step > 0.05)|(vcw1==vcwmin):
			...
			self.setvcw(vcw1)		#constraints vcw set to current value
	...
	def valid_v(self,lv,N):
		...
		for c in self.c:		# for each constraints 
			...
			if (c.type != 'RSS') & (c.type != 'Exclude') :
				...
				DDB,TB   =  c.valid_v(lv)#.reshape(2,len(lv)/4,pow(2,self.ndim))
				TT.append(TB)
				...
				CC=TB[2,:]
				DD=TB[3,:]
				TAB=(((~AA)*(~BB)*(DD))+(BB*(~CC)*(~DD))+(AA*BB*(~CC)))
...
#		if  ((nboxamb==0)&(nbox==0)) and len(self.dlayer[l][0].box) == 0:				
		if  ((nboxamb==0)&(nbox==0)) and len(self.dlayer[l][0].box) == 0:				
			if self.iter < 25:
				...
				self.iter = 0	
				self.dlayer[l-1]=self.dlayer[l] 		# unstack to a lower the layer
		       		l = l - 1
...
#			v=v[0]
			...
			M = ((self.c[c].model.PL0-self.c[c].value)*np.log(10))/(10*self.c[c].model.RSSnp)
			v = 1/(d*S*np.sqrt(2*np.pi))*np.exp(-(((np.log(d)-M)**2)/(2.*(S**2))))
...
#			v = 1/(d*np.sqrt(2*np.pi))*np.exp(-(np.log(d)-mean)**2/(2*std**2))
	...
	def gapdetect(self,l,dlindx):	
		...
		for i in range(self.ndim):
			uni,inv,idd =np.unique(self.dlayer[l][dlindx].bd[:,i],return_inverse=True,return_index=True)
...
#					print 'GAP DETECTED in AXIS',i
					...
					try: 
						...
						clust.append(np.nonzero(uni[gidx[0]]<self.dlayer[l][dlindx].bd[:,i])[0]/2)
						clust.append(np.nonzero(uni[gidx[0]]>self.dlayer[l][dlindx].bd[:,i])[0]/2)
...
##########################################
		...
		if clust != []:	
			...
			for i in range(len(clust)):
				...
				clust_vol=np.sum(np.array(self.dlayer[l][dlindx].vol)[np.unique(clusters)])
...
#					self.pe = self.dlayer[l][dlindx].ctr[np.unique(lclust[0])[peindx]]
					...
					self.pe=np.mean(self.dlayer[l][dlindx].ctr[np.unique(lclust[0])],axis=0)
					pestdmax=np.max(self.dlayer[l][dlindx].ctr[np.unique(lclust[0])])
					pestdmin=np.min(self.dlayer[l][dlindx].ctr[np.unique(lclust[0])])
...
#					self.pe = self.dlayer[l][dlindx].ctr[np.unique(lclust[1])[peindx]]
					...
					self.pe=np.mean(self.dlayer[l][dlindx].ctr[np.unique(lclust[1])],axis=0)
					pestdmax=np.max(self.dlayer[l][dlindx].ctr[np.unique(lclust[1])])
					pestdmin=np.min(self.dlayer[l][dlindx].ctr[np.unique(lclust[1])])
					self.pestd=pestdmax-pestdmin
