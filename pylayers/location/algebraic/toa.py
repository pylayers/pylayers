...
class ToALocation(object):
    ...
    def TSToALocation(self, P0, RN, ToA, ToAStd, Niter):
        ...
        for i in arange(Niter):
            ...
            # Construct the matrix A (see theory)
            A = ((outer(P, ones(RNnum)) - RN) / sqrt(
                sum((outer(P, ones(RNnum)) - RN) ** 2, axis=0))).T

            # Construct the Covariance Matrix
            C = diag((RoAStd[:, 0]) ** 2)

            # Construct the vector D (see theory)
            D = RoA - sqrt((sum((outer(P, ones(
                RNnum)) - RN) ** 2, axis=0)).reshape(shape(RoA)))
    ...
    def MLToALocate(self, P, P0, RN, ToA, ToAStd):
        ...
        P = optimize.fmin(self.ToAOptimizer, P0, args=(RN,
                                                       ToA, ToAStd), xtol=1e-10, ftol=1e-10)
