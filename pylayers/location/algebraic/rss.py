...
class RSSLocation(object):
    ...
    def getPLmean(self, RN, P, PL0, d0, RSSnp):
        ...
        """
        shRN = shape(RN)
        RNmP = (sqrt(sum((RN - P) ** 2, axis=0))).reshape((shRN[
            1], 1))                    # distance between AN and P
    ...
    def getPL(self, RN, P, PL0, d0, RSSnp, RSSStd):
        ...
        shPLmean = shape(PLmean)
        Xrand = RSSStd * randn(shPLmean[0], shPLmean[1])
    ...
    def getRange(self, RN, PL0, d0, RSS, RSSnp, RSSStd, Rest):
        ...
        """
        S = -(log(10) / 10) * RSSStd / \
            RSSnp                    # STD of ranges distribution
        M = (log(10) / 10) * (PL0 - RSS) / RSSnp + \
            log(d0)        # Mean of ranges distribution
    ...
    def getRangeStd(self, RN, PL0, d0, RSS, RSSnp, RSSStd, Rest):
        ...
        """
        S = -(log(10) / 10) * RSSStd / RSSnp                                    # STD of ranges distribution
        M = (log(10) / 10) * (PL0 - RSS) / RSSnp + \
            log(d0)             # Mean of ranges distribution

        if lower(Rest) == 'mode':
            return          sqrt((exp(2 * M - 2 * S ** 2)) * (-exp(-S ** 2) + 1))
        elif lower(Rest) == 'median':
            return          sqrt((exp(2 * M + S ** 2)) * (exp(S ** 2) - 1))
        elif lower(Rest) == 'mean':
            return          sqrt((exp(2 * M + 3 * S ** 2)) * (exp(S ** 2) - 1))
        else:
            return          sqrt((exp(2 * M + S ** 2)) * (exp(S ** 2) - 1))
            print           "No \"%s\" defined estimator" % Rest
    ...
    def TLSRSSLocate(self, RN, PL0, d0, RSS, RSSnp, RSSStd, Rest):
        ...
        if log10(cond(A)) >= max(self.getRangeStd(RN, PL0, d0, RSS, RSSnp, RSSStd, Rest)):
            f = f + 1
            for i in range(n - rA):
                u = where(J == max(J))
                J[u] = 0
    ...
    def WLSRSSLocate(self, RN, PL0, d0, RSS, RSSnp, RSSStd, Rest):
        ...
        # Apply LS operator
        P = 0.5 * dot(linalg.inv(dot(A.T, dot(linalg.inv(C),
    ...
    def MLIRSSLocate(self, P, P0, RN, PL0, d0, RSS, RSSnp, RSSStd, Rest):
        ...
        P = optimize.fmin(self.IRSSOptimizer, P0, args=(RN, PL0,
                                                        d0, RSS, RSSnp, RSSStd, Rest), xtol=1e-10, ftol=1e-10)
    ...
    def DRSSOptimizer1(self, P, RN, PL0, d0, RSS, RSSnp, RSSStd):
        ...
        RNmP = RN - outer(P, ones(RNnum))
        mRNmP = (sqrt(diag(dot(RNmP.T, RNmP)))).reshape(RNnum, 1)
        tk = (RSS - PL0 - 10 * RSSnp * log10(mRNmP / d0)) ** 2
        uk = tk / (2 * RSSStd ** 2)
        suk = uk.sum(axis=0)
    ...
    def MLDRSSLocate(self, P, P0, RN, PL0, d0, RSS, RSSnp, RSSStd):
        ...
        P = optimize.fmin(self.DRSSOptimizer, P0, args=(
            RN, PL0, d0, RSS, RSSnp, RSSStd), xtol=1e-10, ftol=1e-10)
    ...
    '''def SDPRSSLocate(self, RN, PL0, d0, RSS, RSSnp, RSSStd, Rest):
            ...
            for i in range(nrss):
                X0=cvxm.matrix([[Im, -cvxm.transpose(RN[:,i])],[-RN[
                    :,i], cvxm.transpose(RN[:,i])*RN[:,i]]])
    ...
    def SDPRSSLocate(self, RN, PL0, d0, RSS, RSSnp, RSSStd, Rest):
        ...
        for i in range(nrss):
            ...
            prob.constr.append(-RSSStd[i, 0] * t[i] < Si[i] *
                               cvxm.trace(X0 * Y) - 1)
            prob.constr.append(RSSStd[i, 0] * t[i] > Si[i] *
                               cvxm.trace(X0 * Y) - 1)
    ...
    def CRBRSSLocate(self, P, RN, PL0, d0, RSSnp, RSSStd):
        ...
        num     = sum((1+S**2)/((
            S**2)*mRNmP**2),axis=0)[0]     # the numerator of the CRLB

        div1    = sum(((1+S[:,0]**2)*RNmP**2)/((S[:,0]**2)* \
            mRNmP**4),axis=1).reshape(shP)
        don1    = div1.prod(axis=0)[0]          # first term of the doniminator

        div2    = sum(((1+S[:,0]**2)*(RNmP.prod(axis=0)))/((S[:,0]
            **2)*mRNmP**4),axis=0)
