...
class Signature(object):
    ...
    def image(self,tx):
        ...
        c    = (2./alpha)*(pa[0,:]*(pa[1,:]-pb[1,:])**2+pa[1,:]*(pa[1,:]-pb[1,:])*(pb[0,:]-pa[0,:]))
        d    = (2./alpha)*(pa[1,:]*(pb[0,:]-pa[0,:])**2+pa[0,:]*(pa[1,:]-pb[1,:])*(pb[0,:]-pa[0,:]))
        ...
        # if diffraction 
        if len(usig)>0:
            v     = np.kron(2*usig,np.array([1,0]))+np.kron(2*usig+1,np.array([0,1]))
            dm2[v] = 0
        ...
        y    = np.hstack((v0,np.kron(c1,np.array([1,0]))+np.kron(d1,np.array([0,1]))))
    ...
    def backtrace(self,tx,rx,M):
        ...
        while ( ((beta<=1)&(beta>=0)) & (k<N)) :
            ...
            l1    = np.hstack((I2,z0,pa[:,-(k+1)].reshape(2,1)-pb[:,-(k+1)].reshape(2,1)))
            T     = np.vstack((l0,l1))
