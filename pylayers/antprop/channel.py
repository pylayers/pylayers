...
class Ctilde(object):
    """
    This class contains the 4 FUsignal from the Ctilde matrices of all rays.
    ...
    def choose(self): 
        ...
        filefield = FD.askopenfilename(filetypes = [("Files field  ","*.field"),\
                    ("All", "*")], title="Please choose a .field file", initialdir=tuddir)
    ...
    def load(self,filefield,transpose=False):
        ...
        print "nfreq : ",nfreq
        if nfreq == 0:
            print " Error : incorrect number of frequency points .field file corrupted " 
            self.fail = True
            return
        else:
            n     = nray*nfreq
    ...
    def doadod(self,cmap=plt.cm.hot_r,s=30):
        ...
        plt.subplot(121)
        plt.scatter(dod[:,0]*al,dod[:,1]*al,s=s,c=col,cmap=cmap,edgecolors='none')
        ...
        plt.subplot(122)
        plt.scatter(doa[:,0]*al,doa[:,1]*al,s=30,c=col,cmap=plt.cm.hot_r,edgecolors='none')
        plt.axis((0,180,0,360))
        plt.xticks(fontsize=20)
        plt.yticks(fontsize=20)
        #b=colorbar()
    ...
    def vec2scal(self):
        ...
        with the antennas transfer function
...
class VectChannel(Ctilde):
    """
    Full vectorial representation of the propagation channel associated with a RadioLink
...
#              self.rang = tmp[0:nray,:]
    ...
    def show3_old(self,id=0):
        ...
        k - index of filetra
        This function provides a complete ray tracing show of the channel structure
        The rays are color coded as a fonction of energy
        """
        E  = self.Ctt.energy()+self.Ctp.energy()+self.Cpt.energy()+self.Cpp.energy()
        u  = argsort(E)
        v  = u[-1::-1]
        Es = E[v]
    ...
    def show3(self,seuildb=100):
        ...
        Summary 
        -------
        This function provides a complete ray tracing visualization of the channel structure
        The rays are color coded as a fonction of energy
    ...
    def mobility(self,v,dt):
        ...
        Notes      
        ------
        Calculate new channel field from the old one and v(terminal vitese) and dt(time of deplacement)
...
class ScalChannel(object):
    ...
    def doddoa(self):
        ...
        plt.subplot(121)
        plt.scatter(dod[:,0]*al,dod[:,1]*al,s=15,c=col,cmap=plt.cm.gray_r,edgecolors='none')
        a=colorbar()
        #a.set_label('dB')
        ...
        title('DoD')
        plt.subplot(122)
        plt.scatter(doa[:,0]*al,doa[:,1]*al,s=15,c=col,cmap=plt.cm.gray_r,edgecolors='none')
        b=colorbar()
        b.set_label('dB')
        plt.title('DoA')
