...
"""
 .. todo:: 
     ...
     Future evolution will gradually suppress the need to call PulsRay
...
class Simul(object):
    ...
    def deepsave(self):
        ...
        sigui = multenterbox('','Simulation file',
            ('filesimul','filestr','fileantTx','fileantRx'),
...
#
        if "frequency" in sections:
            self.freq = np.linspace(float(self.config.get("frequency","fmin"))\
                                ,float(self.config.get("frequency","fmax"))\
                                ,float(self.config.get("frequency","Nf")),endpoint=True)
...
#
            ...
            for itx in self.config.options("output"):
                ...
                if "field" in secout:
                    ...
                    for kt in keys_field:
                        self.dfield[int(itx)][int(kt)] = output.get("field",kt) 
    ...
    def show(self,itx=[-1],irx=[-1],furniture=True,s=8,c='b',traj=False,num=False):
        ...
        if irx[0]==-1:
            ax.scatter(self.rx.position[:,0],self.rx.position[:,1],c='b',s=s,alpha=0.5)
            ...
            for k in irx:
                ...
                ax.scatter(self.rx.position[k-1,0],self.rx.position[k-1,1],c='b',s=s,alpha=0.5)
                if num:
                    ax.text(self.rx.position[k-1,0],self.rx.position[k-1,1],str(k),color='blue')

        if itx[0]==-1:
            ax.scatter(self.tx.position[:,0],self.tx.position[:,1],c='r',s=s)
            if num:
                for k in range(302):
            ...
            if traj:
                cpt = 1 
            for k in itx:
                ax.scatter(self.tx.position[k-1,0],self.tx.position[k-1,1],c=c,s=s,linewidth=0)
                if num:
                    if traj:
    ...
    def scatter(self,itx,irx,values,
        ...
        plt.axis(vaxis)
        b1 = ax.scatter(xtx,ytx,s=s,c=values,cmap=cmap,linewidths=0,vmin=vmin,vmax=vmax)
...
#                print self.dcir[itx][irx]
    ...
    def filtray(self,itx,irx,tau0,tau1,col='b'):
        ...
        plt.title('Tx'+str(itx)+'-Rx'+str(irx)+' : '+str(tau0)+' < tau < '+str(tau1))
    ...
    def showray(self,itx,irx,iray=np.array([]),fig=[],ax=[]):
        ...
        if len(iray)==1:
            ...
            plt.title('Tx'+str(itx)+'-Rx'+str(irx)+' '+str(min(iray))+' '+str(max(iray)))
    ...
    def run(self,runlevel=5):
        ...
        if (self.progress <5) & (runlevel>4):
            if "waveform" in self.config.sections():
                ...
                self.CIR([self.ntx],[self.nrx],store_level=16+8+4+2+1,alpha=alpha)
        ...
        for irx in srx:
            ...
            if not tracingabort:
                if irx not in self.dtud[itx].keys():
                    ...
                    gt = self.gt(itx,irx)
                    gt.save(self.dtud[itx][irx],self.dtang[itx][irx],self.drang[itx][irx])
                    fd = open(outfilename,"w")
                    self.output[itx].write(fd)
                    fd.close()
                ...
                if ((irx not in self.dcir[itx].keys())|(cirforce==True)):   
                    ...
                    if "waveform" in self.config.sections():
                        ...
                        self.wav = wvf.Waveform(self.wparam) 
                        alpha    = np.sqrt(1./30.0)
                        self.CIR([itx],[irx],store_level=16+8+4+2+1,alpha=alpha)
